...
def recherche():
    ...
    if not recherche_input:
        # Si aucun terme de recherche n'est saisi, rediriger vers la page de résultats sans terme de recherche
...
def resultats(query):
    if query:
        # Effectuer la recherche en fonction du terme de recherche (query) et récupérer les articles correspondants
        db = get_db()
        articles = db.get_articles(query)
    else:
        # Si aucun terme de recherche n'est spécifié, récupérer tous les articles
        db = get_db()
